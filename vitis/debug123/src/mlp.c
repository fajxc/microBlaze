...
#define REG6 (BASE + 0x18)  // partial (now prod0)
...
{
    // HW reads (after DONE)
	int32_t hw_x0  = (int32_t)Xil_In32(REG4); // x0
	int32_t hw_raw_h0  = (int32_t)Xil_In32(REG5);
    int32_t hw_w2_used = (int32_t)Xil_In32(REG7); // reg7 is w2, reg5 is rawh0. free = reg4,6
    int32_t hw_w1 = (int32_t)Xil_In32(REG6); // w1_00

    int32_t sw_x0     = (int32_t)input_image[0];     // pixel 0 (0..255)
    int32_t sw_w1_00  = (int32_t)w1[0][0];           // signed int8 promoted
    int32_t sw_prod0  = sw_w1_00 * sw_x0;
    int32_t sw_b1_0   = (int32_t)b1[0];
    ...
    xil_printf("L1DBG: SW x0=%d w1_00=%d prod0=%d | HW x0=%d w1_00=%d\r\n",
                   sw_x0, sw_w1_00, sw_prod0, hw_x0, hw_w1);
    ...
    xil_printf("DBG2: HW raw h0=%d | SW raw h0=%d\r\n", hw_raw_h0, sw_raw_h0);

    //xil_printf("DBG2: HW term0=%d | SW prod0=%d\r\n", hw_prod, sw_prod0);
